package pt.leirt.mpd;

import org.junit.jupiter.api.Test;
import pt.leirt.mpd.products.Promo;
import pt.leirt.mpd.products.SmartPhone;

import static org.junit.jupiter.api.Assertions.assertEquals;

public class PromoTests {

    @Test
    public void checkCorrectPromoAttributesTest() {
        var promoSansung = new Promo(StoreDB.samsungS23, 20);
        var expectedPrice = 960;
        var expectedName = "Promo " + StoreDB.samsungS23.getName();
        var expectedCategory = StoreDB.samsungS23.getCategory();
        
        
        assertEquals(expectedName, promoSansung.getName());
        assertEquals(expectedCategory, promoSansung.getCategory());
        assertEquals(expectedPrice, promoSansung.getPrice());
        assertEquals(20, promoSansung.getDiscount());
        assertEquals(SmartPhone.class, promoSansung.getWrappee().getClass());
    }
}
